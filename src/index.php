				...
				if ($file != "." && $file != ".." && substr($file, 0, 1) != ".")
				{
					...
					if (file_exists("$currentdir/" . $file . "/folder.jpg"))
					{
						$dirs[] = array(
							...
							"html" => "<li><a href='?dir=" .ltrim($_GET['dir'] . "/" . $file, "/") . "'><em>" . padstring($file, $label_max_length) . "</em><span></span><img src='" . GALLERY_ROOT . "createthumb.php?filename=" . urlencode($currentdir) . "/" . urlencode($file) . "/folder.jpg&amp;size=$thumb_size'  alt='$label_loading' /></a></li>");
					}  else
					{
					// Set thumbnail to first image found (if any):
						...
						$dirs[] = array(
							...
							"html" => "<li><a href='?dir=" . ltrim($_GET['dir'] . "/" . $file, "/") . "'><em>" . padstring($file, $label_max_length) . "</em><span></span><img src='" . GALLERY_ROOT . "createthumb.php?filename=$thumbdir/" . urlencode($file) . "/" . urlencode($firstimage) . "&amp;size=$thumb_size'  alt='$label_loading' /></a></li>");
						} else {
...
// 3. LOAD FILES
	        	if ($file != "." && $file != ".." && $file != "folder.jpg" && substr($file, 0, 1) != ".")
		  		{
		  			// JPG, GIF and PNG
		  			if (preg_match("/.jpg$|.gif$|.png$|.mp4$|.flv$/i", $file))
		  			{
						//Read EXIF
						...
						checkpermissions($currentdir . "/" . $file);
			  			$files[] = array (
							...
							"size" => filesize($currentdir . "/" . $file),
				  			"html" => "<li><a href='" . $currentdir . "/" . $file . "' rel='lightbox[billeder]' title='$img_captions[$file]'><span></span><img src='" . GALLERY_ROOT . "createthumb.php?filename=" . urlencode($thumbdir) . "/" . urlencode($file) . "&amp;size=$thumb_size' alt='$label_loading' /></a><em>" . padstring($file, $label_max_length) . "</em></li>");
		  			}
					// Other filetypes
					$extension = "";
